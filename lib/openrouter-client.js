import { ANALYSIS_JSON_SCHEMA, REWRITE_AUDIT_JSON_SCHEMA } from "./analysis-schema.js";
import {
  ModelOutputError,
  rewritePreservesNumbers,
  validateAndEnrichAnalysis,
  validateRewriteAudit,
} from "./model-validation.js";
import {
  ANALYSIS_SYSTEM_PROMPT,
  PROMPT_VERSION,
  REWRITE_AUDIT_SYSTEM_PROMPT,
  createAnalysisUserPrompt,
  createRewriteAuditUserPrompt,
} from "./prompts.js";
...
export async function analyseWithOpenRouter(input, preferences, fetchImplementation = fetch) {
  ...
  const firstCall = await callStructuredOutput({
    ...requestOptions,
    schemaName: "angleprobe_analysis",
    schema: ANALYSIS_JSON_SCHEMA,
    systemPrompt: ANALYSIS_SYSTEM_PROMPT,
    userPrompt: createAnalysisUserPrompt(input),
  });
  const usageStages = [{ stage: "analysis", usage: firstCall.usage }];

  let analysis;
  try {
    analysis = validateAndEnrichAnalysis(firstCall.output, input.text);
  } catch (error) {
  ...
  if (
    analysis.neutral_rewrite !== null &&
    normalizeForComparison(analysis.neutral_rewrite) === normalizeForComparison(input.text)
  ) {
    analysis = { ...analysis, neutral_rewrite: null };
  }

  let rewriteAudit = { performed: false, approved: false, problems: [] };
  if (analysis.neutral_rewrite !== null) {
    if (!rewritePreservesNumbers(input.text, analysis.neutral_rewrite)) {
      rewriteAudit = { performed: true, approved: false, problems: ["The rewrite changed, added, removed, or reordered a number."] };
      analysis = rejectRewrite(analysis);
    } else {
      const auditCall = await callStructuredOutput({
        ...requestOptions,
        schemaName: "angleprobe_rewrite_audit",
        schema: REWRITE_AUDIT_JSON_SCHEMA,
        systemPrompt: REWRITE_AUDIT_SYSTEM_PROMPT,
        userPrompt: createRewriteAuditUserPrompt(input.text, analysis.neutral_rewrite),
      });
      usageStages.push({ stage: "rewrite_audit", usage: auditCall.usage });
      let audit;
      try {
        audit = validateRewriteAudit(auditCall.output);
      } catch (error) {
        throw new AngleprobeError("invalid_model_output", "This model returned an invalid rewrite audit. Try another model.", error.message);
      }
      rewriteAudit = { performed: true, ...audit };
      if (!audit.approved) analysis = rejectRewrite(analysis);
    }
  }
  ...
  return {
    ...
    model: {
      id: firstCall.resolvedModel ?? preferences.modelId,
      requested_id: preferences.modelId,
      prompt_version: PROMPT_VERSION,
    },
    input: { text: input.text, source_url: input.source_url ?? null },
...
function classifyOpenRouterError(status, body, requireZdr) {
  ...
  if (requireZdr && (lower.includes("zdr") || lower.includes("zero data") || lower.includes("no endpoints") || lower.includes("data retention") || lower.includes("data_collection"))) {
    return new AngleprobeError("zdr_unavailable", "No zero-data-retention endpoint is available for this model. Choose another model or turn off the privacy requirement.", detail);
  }
  if (status === 401 || status === 403) {
    return new AngleprobeError("key_rejected", "OpenRouter rejected this API key. Check it in Settings.", detail);
  }
  if (status === 402) {
    return new AngleprobeError("insufficient_credits", "This OpenRouter account has insufficient credit for the selected model.", detail);
  }
  if (status === 429) {
...
function rejectRewrite(analysis) {
  return {
    ...analysis,
    neutral_rewrite: null,
    limitations: addLimitation(analysis.limitations, "A proposed rewrite was rejected by the counter-framing audit."),
  };
}

function addLimitation(limitations, message) {
  if (limitations.includes(message)) return limitations;
  if (limitations.length < 6) return [...limitations, message];
  return [...limitations.slice(0, 5), message];
}
