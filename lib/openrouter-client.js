...
async function callStructuredOutput({
  ...
  const requestBody = {
    ...
    // Reasoning tokens share the completion budget on several providers, even
    // when they are excluded from the returned message. Leave enough room for
    // the model to think and still emit the analysis JSON.
    max_tokens: completionBudgetFor(reasoningEffort),
  ...
  try {
    ...
    if (!response.ok) throw classifyOpenRouterError(response.status, {}, requireZdr);
  ...
  if (!content) {
    if (choice?.finish_reason === "length") {
      throw new AngleprobeError(
        "completion_limit",
        "The model used its response budget before producing the analysis. Retry with a lower reasoning level or choose another model.",
      );
    }
    if (message?.refusal) {
      throw new AngleprobeError(
        "model_refusal",
        "The model declined this analysis. Try another model or a smaller passage.",
      );
    }
    throw new AngleprobeError("invalid_model_output", "This model did not return a usable structured analysis. Retry or choose another model.");
  }
...
function parseStructuredContent(content) {
  ...
  try {
    ...
    // Some providers prepend a short sentence despite JSON mode. Extract only
    // a complete top-level object; validation still controls what is displayed.
...
function completionBudgetFor(effort) {
  // Excessively large budgets make some reasoning models deliberate until a
  // provider gateway times out. Scale the allowance without inviting runaway
  // completions.
  if (["high", "xhigh", "max"].includes(effort)) return 8_000;
  if (effort === "medium") return 6_000;
  if (effort === "low") return 4_000;
  if (["none", "minimal"].includes(effort)) return 3_000;
  return 5_000;
}
